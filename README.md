...
## Nick tracking
- When the bot runs it also maintains a `NickList` worksheet in the same spreadsheet.
- Columns: `Nick Name`, `Times Seen`, `First Seen`, `Last Seen`. Each nick increments `Times Seen` and updates the timestamp for `Last Seen` (first run sets `First Seen`).
